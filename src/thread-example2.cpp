...
// Additionally, it will solve the race condition issue present in example1.
...
public:
    ...
    void increment() {
        std::lock_guard<std::mutex> lock(mtx); // Lock the mutex to protect shared state
        // This ensures that only one thread can access the shared value at a time
        ...
        // Unlock the mutex (optional, as lock_guard will automatically unlock when it goes out of scope)
    }
